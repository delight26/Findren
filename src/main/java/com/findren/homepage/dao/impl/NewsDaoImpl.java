...
public class NewsDaoImpl implements NewsDao {
	...
	public List<NewsBoard> getNewsBoardList(int startRow, int PAGE_SIZE) {
		newsparam = new MapSqlParameterSource("startRow", startRow).addValue("PAGE_SIZE",
				PAGE_SIZE);
		sql = "select * from findren_write_02_board order by wr_datetime desc limit :startRow, :PAGE_SIZE";
	...
	public void insertNewsBoard(NewsBoard nb) {
		...
		sql = "insert into findren_write_02_board (wr_id, wr_subject, wr_content, mb_id, wr_name, wr_link1, wr_link2, wr_link1_hit, wr_link2_hit,"
	...
	public void newsWatchUpdate(int watch, int no) {
		...
		sql = "update findren_write_02_board set wr_hit = :watch where wr_id = :no";
		namedParameterJdbcTemplate.update(sql, newsparam);
	...
	public NewsBoard newsContent(int no) {
		...
		sql = "select * from findren_write_02_board where wr_id = :no";
		return namedParameterJdbcTemplate.query(sql, newsparam, dm.getNewsBoardRMRSE());
	}
	...
	public Integer newsNextNo(int no) {
		newsparam = new MapSqlParameterSource("no", no);
		sql = "SELECT max(wr_id) FROM findren_write_02_board nb WHERE wr_id < :no;";
	...
	public Integer newsPreNo(int no) {
		newsparam = new MapSqlParameterSource("no", no);
		sql = "SELECT min(wr_id) FROM findren_write_02_board nb WHERE wr_id > :no;";
		return namedParameterJdbcTemplate.queryForObject(
				sql, newsparam, Integer.class);
	}
	...
	public void newsDelete(int no) {
		newsparam = new MapSqlParameterSource("no", no);
		sql = "delete from findren_write_02_board where wr_id = :no";
		namedParameterJdbcTemplate.update(sql, newsparam);
	}
	...
	public void updateNewsBoard(NewsBoard nb) {
		...
		if(!nb.getWr_file1().isEmpty() && !nb.getWr_file2().isEmpty()){
			sql = "update findren_write_02_board set wr_subject = :wr_subject, wr_content = :wr_content, wr_link1 = :wr_link1, wr_link2 = :wr_link2,"
	...
	public Integer maxNum() {
		newsparam = new MapSqlParameterSource("findren_write_02_board", "findren_write_02_board");
		sql = "select max(wr_num) from findren_write_02_board";
		return namedParameterJdbcTemplate.queryForObject(sql, newsparam, Integer.class);
	}
}
