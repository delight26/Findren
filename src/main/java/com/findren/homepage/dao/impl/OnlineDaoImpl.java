...
@Repository
public class OnlineDaoImpl implements NoticeDao {
	...
	{
		noticeparam = new MapSqlParameterSource("findren_write_05_board", "findren_write_05_board");
	...
	public Integer noticeNextNo(int no) {
		noticeparam = new MapSqlParameterSource("no", no);
		sql = "SELECT max(wr_id) FROM findren_write_05_board nb WHERE wr_id < :no;";
	...
	public Integer noticePreNo(int no) {
		noticeparam = new MapSqlParameterSource("no", no);
		sql = "SELECT min(wr_id) FROM findren_write_05_board nb WHERE wr_id > :no;";
		return namedParameterJdbcTemplate.queryForObject(
				sql, noticeparam, Integer.class);
	}
