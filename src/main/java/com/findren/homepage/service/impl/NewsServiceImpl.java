...
import com.findren.homepage.service.NewsService;
...
public class NewsServiceImpl implements NewsService {
	...
	@Override
	public void newsWriteResult(MultipartHttpServletRequest request, String path, HttpSession session)
	...
	public void newsContent(HttpServletRequest request) {
		...
		NDao.newsWatchUpdate(watch + 1, no);
		NewsBoard nb = NDao.newsContent(no);
	...
	public void newsNext(HttpServletRequest request) {
		int no = Integer.valueOf(request.getParameter("no"));
		int pageNum = Integer.valueOf(request.getParameter("pageNum"));
		Integer nextNo = NDao.newsNextNo(no);
		if (nextNo == null) {

		} else {
			NewsBoard nb = NDao.newsContent(nextNo);
			NDao.newsWatchUpdate(nb.getWr_hit() + 1, nextNo);
			request.setAttribute("pageNum", pageNum);
			request.setAttribute("nb", nb);
		}
	}
	...
	public void newsPre(HttpServletRequest request) {
		int no = Integer.valueOf(request.getParameter("no"));
		int pageNum = Integer.valueOf(request.getParameter("pageNum"));
		Integer nextNo = NDao.newsPreNo(no);
		if (nextNo == null) {
	...
	@Override
	public void newsUpdateResult(MultipartHttpServletRequest request, String path, HttpSession session)
			throws IllegalStateException, IOException {
		request.setCharacterEncoding("utf-8");
		MultipartFile multipartFile1 = request.getFile("file1");
		MultipartFile multipartFile2 = request.getFile("file2");
